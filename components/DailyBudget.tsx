...
export function DailyBudget({
  ...
  return (
    <div className="bg-white rounded-2xl border border-stone-200/80 p-4 sm:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.02)] space-y-4 sm:space-y-5 w-full min-w-0">
      {/* Header bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 min-w-0">
        ...
        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] sm:text-xs font-medium border bg-stone-50 border-stone-200/70 text-zinc-600 self-start sm:self-auto">
      ...
      <div className="bg-stone-50/70 rounded-xl p-3.5 sm:p-5 border border-stone-200/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 min-w-0">
        <div className="min-w-0">
          <span className="text-[11px] sm:text-xs font-medium text-zinc-500 block mb-1">
            Jatah Aman Konsumsi Hari Ini
          </span>
          <div className="flex items-baseline gap-1.5 sm:gap-2">
            <span className="font-sans font-bold text-2xl sm:text-3xl md:text-4xl text-zinc-900 tabular-nums break-words">
              {formatCurrency(dailyAllowance)}
            </span>
            <span className="text-xs text-zinc-500 font-medium">/ hari</span>
          </div>
          <p className="text-xs text-zinc-500 mt-1 leading-relaxed">
            Target ideal: <strong className="text-zinc-800 font-semibold">{formatCurrency(dailyTarget)}/hari</strong> ({totalDays} hari siklus).
          </p>
        </div>
      ...
      <div className="space-y-2 min-w-0">
        ...
        <div className="flex justify-between items-center text-[10px] sm:text-[11px] text-zinc-500 pt-0.5 gap-2">
